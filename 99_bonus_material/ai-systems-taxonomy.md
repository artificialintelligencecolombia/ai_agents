...
**Version:** 2.0  **Last revised:** 2026-07-07  **Status:** Working reference, grounded to public industry and research sources (see *Sources*).
...
| **Embedding Model** | A model that converts text/data into vectors for similarity search; the backbone of all retrieval. |
...
| **Tools / Function calling** | External APIs, code execution, or actions the model can invoke to read or change the world. |
...
An agent is a system where the LLM **directs its own process and tool use**, driven by environment feedback in a loop, rather than following predefined code paths. Use only when the task genuinely needs open-ended, adaptive decision-making.
...
| **Agent / orchestration frameworks** | Implementation tools (LangGraph, CrewAI, AutoGen, Anthropic Agent SDK, LlamaIndex, Semantic Kernel). **These are how systems are *built*, not categories of system** — do not classify a solution *as* its framework. |
...
3. **Is knowledge retrieved?** If external data is fetched to ground answers, it is in the *RAG family* (Layer 5) — then place it on the naive→agentic maturity spectrum.
4. **Who owns the control flow?** Fixed, developer-defined paths → *workflow*. The model decides its next step from feedback → *agent* (Layer 7).
5. **How many decision-makers?** One agent → *single-agent*. Several coordinating → *multi-agent / agentic AI* (Layer 8).
...
**v2.0 (this version)** corrects and extends v1.0:
...
- **Berkeley AI Research (BAIR)** — *The Shift from Models to Compound AI Systems* (Zaharia, Khattab, et al., Feb 18 2024). Origin of the "compound AI system" definition.
- **Anthropic** — *Building Effective Agents* (Schluntz & Zhang, Dec 2024). Source of the augmented-LLM building block, the workflow-vs-agent distinction, and the five workflow patterns; and the *Model Context Protocol* (Nov 2024).
- **Google** — *Agent2Agent (A2A) Protocol* (Apr 2025). Horizontal agent-interoperability standard.
